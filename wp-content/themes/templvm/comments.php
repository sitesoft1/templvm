...
<div id="comments"> <?php // див с этим id нужен для якорьных ссылок на комменты ?>
	<h2>Total comments: <?php echo get_comments_number(); // общие кол-во комментов ?></h2>
	...
	<?php if (comments_open()) { // если комментирование включено для данного поста
		...
		$fields =  array( // разметка текстовых полей формы
			'author' => '<div class="form-group"><label for="author">Name</label><input class="form-control" id="author" name="author" type="text" value="'.esc_attr($commenter['comment_author']).'" size="30" required></div>', // поле Имя
			...
			'url' => '<div class="form-group"><label for="url">Site</label><input class="form-control" id="url" name="url" type="text" value="'.esc_attr($commenter['comment_author_url']).'" size="30"></div>', // поле сайт
			);
		$args = array( // опции формы комментирования
			...
			'comment_field' => '<div class="form-group"><label for="comment">Comment:</label><textarea class="form-control" id="comment" name="comment" cols="45" rows="8" required></textarea></div>', // разметка поля для комментирования
			...
			'logged_in_as' => '<p class="logged-in-as">'.sprintf(__( 'You are logged in as <a href="%1$s">%2$s</a>. <a href="%3$s">Log off?</a>'), admin_url('profile.php'), $user_identity, wp_logout_url(apply_filters('the_permalink',get_permalink()))).'</p>', // разметка "Вы вошли как"
			'comment_notes_before' => '<p class="comment-notes">Your email will not be published.</p>', // Текст до формы
			'comment_notes_after' => '<p class="help-block form-allowed-tags">'.sprintf(__( 'You can use the following <abbr>HTML</abbr> tags: %s'),'<code>'.allowed_tags().'</code>').'</p>', // текст после формы
			'id_form' => 'commentform', // атрибут id формы
			'id_submit' => 'submit', // атрибут id кнопки отправить
			'title_reply' => 'Leave a comment', // заголовок формы
			'title_reply_to' => 'To answer %s', // "Ответить" текст
			'cancel_reply_link' => 'Cancel reply', // "Отменить ответ" текст
			'label_submit' => 'Send', // Текст на кнопке отправить
